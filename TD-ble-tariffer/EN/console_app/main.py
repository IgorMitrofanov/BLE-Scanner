...
async def main(loop, timeout, start_serial, end_serial, test=False):
    if test is False:
        start_serial = int(input('Type start serial (only six numbers): '))
        end_serial = int(input('Type start serial (only six numbers): '))
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(main(loop, timeout=200, start_serial=400043, end_serial=400052)) # test=True 