...
class BleakClientAssistant:
    def __init__(self, device, loop, timeout=15):
        self.device = device
        self.loop = loop
    ...
    async def run(self):
        try:
            async with BleakClient(self.device, timeout=self.timeout, loop=self.loop) as client:
                if client is not None:
                    await client.start_notify(14, self.notification_callback)
                    await client.write_gatt_char(12, b"GA\r", response=True) 
